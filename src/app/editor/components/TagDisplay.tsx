  ...
  return (
    <div className='flex flex-wrap gap-1.5 mb-2'>
      {tags.ingredients.map((tag) => (
        <Badge key={tag} className='flex items-center gap-1 bg-green-100'>
          <Utensils className='size-3' />
          {tag}
        </Badge>
      ))}
      {tags.flavors.map((tag) => (
        <Badge key={tag} className='flex items-center gap-1 bg-orange-100'>
          <Flame className='size-3' />
          {tag}
        </Badge>
      ))}
      {tags.restrictions.map((tag) => (
        <Badge key={tag} className='flex items-center gap-1 bg-sky-100'>
